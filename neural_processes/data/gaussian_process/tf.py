"""Deepmind's gaussian process curve generator
...
class CurveGenerator(object):
  ...
  """
    ...
    """
        # oops their original code missed the + 1
        # requires the minus later to be + 1 too
        num_context = tf.random.uniform(
            shape=[], minval=3, maxval=self._max_num_context + 1, dtype=tf.int32
        )
        ...
        # selected at random
            ...
            num_target = tf.random.uniform(
                shape=(), minval=0, maxval=self._max_num_context - num_context, dtype=tf.int32
            )
            num_total_points = num_context + num_target
            x_values = tf.random.uniform([self._batch_size, num_total_points, self._x_size], -2, 2)

        # Set kernel parameters
        # Either choose a set of random parameters for the mini-batch
        if self._random_kernel_parameters:
            l1 = tf.random.uniform(
                [self._batch_size, self._y_size, self._x_size], 0.1, self._l1_scale
            )
            sigma_f = tf.random.uniform([self._batch_size, self._y_size], 0.1, self._sigma_scale)
        ...
        y_values = tf.matmul(
            cholesky, tf.random.normal([self._batch_size, self._y_size, num_total_points, 1])
        )
